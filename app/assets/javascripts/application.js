...
;(function (global) {
  ...
  gc.details = {
    ...
    addClickEvent: function (node, callback) {
      gc.details.addEvent(node, 'keypress', function (e, target) {
        // When the key gets pressed - check if it is enter or space
        if (gc.details.charCode(e) === gc.details.KEY_ENTER || gc.details.charCode(e) === gc.details.KEY_SPACE) {
          if (target.nodeName.toLowerCase() === 'summary') {
            // Prevent space from scrolling the page
            // and enter from submitting a form
            gc.details.preventDefault(e)
      ...
      gc.details.addEvent(node, 'keyup', function (e, target) {
        if (gc.details.charCode(e) === gc.details.KEY_SPACE) {
      ...
      gc.details.addEvent(node, 'click', function (e, target) {
        callback(e, target)
      })
    },
    ...
    addDetailsPolyfill: function (list, container) {
      ...
      if (gc.details.started) {
        return
      }
      gc.details.started = true
      ...
      gc.details.addClickEvent(container, function (e, summary) {
        if (!(summary = gc.details.getAncestor(summary, 'summary'))) {
          return true
        }
        return gc.details.statechange(summary)
      })
    },
    ...
    destroy: function (node) {
      gc.details.removeEvent(node, 'click')
    },
    ...
    init: function ($container) {
      gc.details.addEvent(document, 'DOMContentLoaded', gc.details.addDetailsPolyfill)
      gc.details.addEvent(window, 'load', gc.details.addDetailsPolyfill)
    }
  }
  global.gc = gc
})(window)
